...
class BookRepositoryCustomImplTest extends AbstractRepositoryTest {
    ...
    void authorExistsInBooks() {
        ...
        var author = authorRepository.findOne(example).orElse(null);
        assertThat(author).isNotNull();
        var exists = bookRepository.authorExistsInBooks(author);
        assertTrue(exists);
        author.setId("not_id");
        author.setName("Not Author");
        exists = bookRepository.authorExistsInBooks(author);
        assertFalse(exists);
    }
    ...
    void genreExistsInBooks() {
        ...
        var genre = genreRepository.findOne(example).orElse(null);
        assertThat(genre).isNotNull();
        var exists = bookRepository.genreExistsInBooks(genre);
        assertTrue(exists);
        genre.setId("not_id");
        genre.setName("Not Genre");
        exists = bookRepository.genreExistsInBooks(genre);
        assertFalse(exists);
    }
    ...
    void updateAuthorInBooks() {
        ...
        var actualBook = bookRepository.findOne(example).orElse(null);
