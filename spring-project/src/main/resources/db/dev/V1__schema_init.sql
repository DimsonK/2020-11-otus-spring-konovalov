...
CREATE SEQUENCE INSTANCE_SEQUENCE START WITH 100;
...
ALTER TABLE BASKETS ADD CONSTRAINT BASKET_TO_USER FOREIGN KEY (USER_ID) REFERENCES USERS(ID);

-- BASKET_BOOKS
DROP TABLE IF EXISTS BASKETS_BOOKS CASCADE;
CREATE TABLE BASKET_BOOKS (
    BOOK_ID BIGINT references BOOKS (ID) on delete cascade,
    BASKET_ID BIGINT references BASKETS (USER_ID),
